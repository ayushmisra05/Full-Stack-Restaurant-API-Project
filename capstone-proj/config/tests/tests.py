...
class SetUpMixin:
    ...
    def setUp(self):
        self.user = self.create_user(username='test@email.com', password='testpasswd')
        self.token = self.get_token(username='test@email.com', password='testpasswd')
...
class SingleBookingViewTest(SetUpMixin, UserMixin, SingleBookingMixin, TestCase):
    ...
    def test_partial_update(self):
        data = {'no_of_guests': 6, 'booking_date': '2023-03-06'}
    ...
    def test_update(self):
        data = {'name': 'will', 'no_of_guests': 6, 'booking_date': '2023-03-06'}
...
class SingleMenuItemViewTest(SetUpMixin, UserMixin, SingleMenuItemMixin, TestCase):
    ...
    def test_update(self):
        data = {'title': 'Apple Juice', 'price': 3.85, 'inventory': 7}
        response = self.client.put(
            reverse('api:menu-detail', kwargs={'pk': self.menu_item.pk}),
            data=json.dumps(data),
            content_type='application/json',
